...
perf.controller('CompareResultsCtrl', [
  ...
  'getSeriesLists', 'calculateStats', 'isReverseTest', 'getResultsMap', 'getCounterMap',
  function CompareResultsCtrl($state, $stateParams, $scope,
                              $rootScope, $location,
                              thServiceDomain, $http, $q,
                              $timeout, getSeriesSummary,
                              math, getSeriesLists, calculateStats,
                              isReverseTest, getResultsMap, getCounterMap) {
    function displayComparision() {
      ...
      getSeriesLists($scope.originalProject,
                    $scope.timeRange,
                    optionCollectionMap,
                    {e10s: $scope.e10s,
                     pgo: $scope.pgo,
                     summaryOnly: true}).then(
        function(originalSeriesData) {
          ...
          getSeriesLists($scope.newProject,
                        $scope.timeRange,
                        optionCollectionMap,
                        {e10s: $scope.e10s,
                         pgo: $scope.pgo,
    ...
    function displayResults(rawResultsMap, newRawResultsMap) {
      ...
      $scope.testList.forEach(function(testName) {
        ...
        $scope.platformList.forEach(function(platform) {
          ...
          var cmap = getCounterMap(testName, rawResultsMap[oldSig], newRawResultsMap[newSig]);
...
perf.controller('CompareSubtestResultsCtrl', [
  '$state', '$stateParams', '$scope', '$rootScope', '$location',
  'thServiceDomain', '$http', '$q', '$timeout', 'getSeriesSummary', 'math',
  'getSeriesLists', 'calculateStats', 'isReverseTest', 'getResultsMap',
  'getCounterMap',
          ...
          verifyRevision($scope.newProject, $scope.newRevision, "new").then(function () {
            $http.get(thServiceDomain + '/api/repository/').then(function(response) {
              ...
              getSeriesLists($scope.originalProject,
                            ...
                            {e10s: $scope.e10s,
                             pgo: $scope.pgo,
                             targetSignature: $scope.originalSignature}).then(
                function (originalSeriesData) {
                  $scope.testList = originalSeriesData.testList;
                  $scope.platformList = originalSeriesData.platformList;
                  return getResultsMap($scope.originalProject,
